...
const Tipo_model_1 = require("./Tipo.model");
const listar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        Tipo_model_1.Tipo.paginate(query, { sort: { [sort]: order }, page, limit: size
        }, (err, result) => {
            return res.send({
                ok: true,
                msg: 'Listado',
...
const crear = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let data = req.body;
        //crea
        let registro = yield Tipo_model_1.Tipo.create(data);
        if (registro) {
            return res.status(200).send({
                ok: true,
                mensaje: "Tipo creado/a con éxito.",
                alerta: 'success',
                registro
            });
        }
...
const actualizar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const registroActualizado = yield Tipo_model_1.Tipo.findByIdAndUpdate({ _id: id }, data, { new: true });
        ...
        return res.status(200).json({
            ok: true,
            mensaje: "Tipo actualizado/a con éxito.",
            alerta: 'success',
            registroActualizado
        });
    }
...
const eliminar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const registro = yield Tipo_model_1.Tipo.findByIdAndDelete(id);
        if (!registro) {
            return res.json({
                ok: false,
                mensaje: "Tipo no encontrado/a.",
            });
        }
        return res.status(200).json({
            ok: true,
            mensaje: "Tipo eliminado/a con éxito.",
        });
    }
    catch (error) {
        console.log("======================================================");
        console.log("===================/Tipo/eliminar===============");
        console.log(error);
        return res.send({
            ok: false,
            mensaje: "Algo salió mal.",

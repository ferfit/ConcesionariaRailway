...
const crear = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let data = req.body;
        //crea
        let registro = yield Producto_model_1.Producto.create(data);
        if (registro) {
            return res.status(200).send({
                ok: true,
                mensaje: "Producto creado/a con éxito.",
                alerta: 'success',
                registro
            });
        }
...
const actualizar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const registroActualizado = yield Producto_model_1.Producto.findByIdAndUpdate({ _id: id }, data, { new: true });
        ...
        return res.status(200).json({
            ok: true,
            mensaje: "Producto actualizado/a con éxito.",
            alerta: 'success',
            registroActualizado
        });
    }
        ...
        console.log("======================================================");
        console.log("===================/Producto/actualizar===============");
        console.log(error);
        return res.status(500).send({
            ok: false,
            mensaje: "Algo salió mal.",
...
const eliminar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const registro = yield Producto_model_1.Producto.findByIdAndDelete(id);
        if (!registro) {
            return res.json({
                ok: false,
                mensaje: "Producto no encontrado/a.",
            });
        }
        return res.status(200).json({
            ok: true,
            mensaje: "Producto eliminado/a con éxito.",
        });
    }
    catch (error) {
        console.log("======================================================");

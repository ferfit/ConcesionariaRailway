...
const Permuta_model_1 = require("./Permuta.model");
const listar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        Permuta_model_1.Permuta.paginate(query, { sort: { [sort]: order }, page, limit: size
        }, (err, result) => {
            return res.send({
                ok: true,
                msg: 'Listado',
...
const crear = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let data = req.body;
        //crea
        let registro = yield Permuta_model_1.Permuta.create(data);
        if (registro) {
            return res.status(200).send({
                ok: true,
                mensaje: "Permuta creado/a con éxito.",
                alerta: 'success',
                registro
            });
        }
...
const actualizar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const registroActualizado = yield Permuta_model_1.Permuta.findByIdAndUpdate({ _id: id }, data, { new: true });
        if (!registroActualizado) {
            return res.status(200).json({
                ok: false,
                mensaje: "Error al actualizar Permuta.",
                alerta: 'danger'
            });
        }
        return res.status(200).json({
            ok: true,
            mensaje: "Permuta actualizado/a con éxito.",
            alerta: 'success',
            registroActualizado
        });
    }
        ...
        console.log("======================================================");
        console.log("===================/Permuta/actualizar===============");
        console.log(error);
        return res.status(500).send({
            ok: false,
            mensaje: "Algo salió mal.",
...
const eliminar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const registro = yield Permuta_model_1.Permuta.findByIdAndDelete(id);
        if (!registro) {
            return res.json({
                ok: false,
                mensaje: "Permuta no encontrado/a.",
            });
        }
        return res.status(200).json({
            ok: true,
            mensaje: "Permuta eliminado/a con éxito.",
        });
    }
    catch (error) {
        console.log("======================================================");
        console.log("===================/Permuta/eliminar===============");
        console.log(error);
        return res.send({
            ok: false,
            mensaje: "Algo salió mal.",
...
//# sourceMappingURL=Permuta.controller.js.map
...
const listar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        Productos_model_1.Productos.paginate(query, { sort: { [sort]: order }, page, limit: size
        }, (err, result) => {
            return res.send({
                ok: true,
                msg: 'Listado',
...
const crear = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let data = req.body;
        //crea
        let registro = yield Productos_model_1.Productos.create(data);
        if (registro) {
            return res.status(200).send({
                ok: true,
                mensaje: "Productos creado/a con éxito.",
                alerta: 'success',
                registro
            });
        }
...
const actualizar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const registroActualizado = yield Productos_model_1.Productos.findByIdAndUpdate({ _id: id }, data, { new: true });
        ...
        return res.status(200).json({
            ok: true,
            mensaje: "Productos actualizado/a con éxito.",
            alerta: 'success',
            registroActualizado
        });
    }
...
const eliminar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const registro = yield Productos_model_1.Productos.findByIdAndDelete(id);
        if (!registro) {
            return res.json({
                ok: false,
                mensaje: "Productos no encontrado/a.",
            });
        }
        return res.status(200).json({
            ok: true,
            mensaje: "Productos eliminado/a con éxito.",
        });
    }
    catch (error) {
        console.log("======================================================");

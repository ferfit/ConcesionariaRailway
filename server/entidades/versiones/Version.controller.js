...
const listar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        Version_model_1.Version.paginate(query, { sort: { [sort]: order }, page, limit: size
        }, (err, result) => {
            return res.send({
                ok: true,
                msg: 'Listado',
...
const crear = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let data = req.body;
        //crea
        let registro = yield Version_model_1.Version.create(data);
        if (registro) {
            return res.status(200).send({
                ok: true,
                mensaje: "Version creado/a con éxito.",
                alerta: 'success',
                registro
            });
        }
...
const actualizar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const registroActualizado = yield Version_model_1.Version.findByIdAndUpdate({ _id: id }, data, { new: true });
        if (!registroActualizado) {
            return res.status(200).json({
                ok: false,
                mensaje: "Error al actualizar Version.",
                alerta: 'danger'
            });
        }
        return res.status(200).json({
            ok: true,
            mensaje: "Version actualizado/a con éxito.",
            alerta: 'success',
            registroActualizado
        });
    }
        ...
        console.log("======================================================");
        console.log("===================/Version/actualizar===============");
        console.log(error);
        return res.status(500).send({
            ok: false,
            mensaje: "Algo salió mal.",
...
const eliminar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const registro = yield Version_model_1.Version.findByIdAndDelete(id);
        if (!registro) {
            return res.json({
                ok: false,
                mensaje: "Version no encontrado/a.",
            });
        }
        return res.status(200).json({
            ok: true,
            mensaje: "Version eliminado/a con éxito.",
        });
    }
    catch (error) {
        console.log("======================================================");
        console.log("===================/Version/eliminar===============");
        console.log(error);
        return res.send({
            ok: false,
            mensaje: "Algo salió mal.",

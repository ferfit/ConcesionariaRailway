...
const Saludo_model_1 = require("./Saludo.model");
const listar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        Saludo_model_1.Saludo.paginate(query, { sort: { [sort]: order }, page, limit: size
        }, (err, result) => {
            return res.send({
                ok: true,
                msg: 'Listado',
...
const crear = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let data = req.body;
        //crea
        let registro = yield Saludo_model_1.Saludo.create(data);
        if (registro) {
            return res.status(200).send({
                ok: true,
                mensaje: "Saludo creado/a con éxito.",
                alerta: 'success',
                registro
            });
        }
...
const actualizar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const registroActualizado = yield Saludo_model_1.Saludo.findByIdAndUpdate({ _id: id }, data, { new: true });
        if (!registroActualizado) {
            return res.status(200).json({
                ok: false,
                mensaje: "Error al actualizar Saludo.",
                alerta: 'danger'
            });
        }
        return res.status(200).json({
            ok: true,
            mensaje: "Saludo actualizado/a con éxito.",
            alerta: 'success',
            registroActualizado
        });
    }
...
const eliminar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const registro = yield Saludo_model_1.Saludo.findByIdAndDelete(id);
        if (!registro) {
            return res.json({
                ok: false,
                mensaje: "Saludo no encontrado/a.",
            });
        }
        return res.status(200).json({
            ok: true,
            mensaje: "Saludo eliminado/a con éxito.",
        });
    }
    catch (error) {
        console.log("======================================================");

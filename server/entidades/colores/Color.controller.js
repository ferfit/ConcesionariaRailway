...
const listar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        Color_model_1.Color.paginate(query, { sort: { [sort]: order }, page, limit: size
        }, (err, result) => {
            return res.send({
                ok: true,
                msg: 'Listado',
...
const crear = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let data = req.body;
        //crea
        let registro = yield Color_model_1.Color.create(data);
        if (registro) {
            return res.status(200).send({
                ok: true,
                mensaje: "Color creado/a con éxito.",
                alerta: 'success',
                registro
            });
        }
...
const actualizar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const registroActualizado = yield Color_model_1.Color.findByIdAndUpdate({ _id: id }, data, { new: true });
        ...
        return res.status(200).json({
            ok: true,
            mensaje: "Color actualizado/a con éxito.",
            alerta: 'success',
            registroActualizado
        });
    }
...
const eliminar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const registro = yield Color_model_1.Color.findByIdAndDelete(id);
        if (!registro) {
            return res.json({
                ok: false,
                mensaje: "Color no encontrado/a.",
            });
        }
        return res.status(200).json({
            ok: true,
            mensaje: "Color eliminado/a con éxito.",
        });
    }
    catch (error) {
        console.log("======================================================");
        console.log("===================/Color/eliminar===============");
        console.log(error);
        return res.send({
            ok: false,
            mensaje: "Algo salió mal.",

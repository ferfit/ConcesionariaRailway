...
const listar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        Marca_model_1.Marca.paginate(query, { sort: { [sort]: order }, page, limit: size
        }, (err, result) => {
            return res.send({
                ok: true,
                msg: 'Listado',
...
const crear = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let data = req.body;
        //crea
        let registro = yield Marca_model_1.Marca.create(data);
        if (registro) {
            return res.status(200).send({
                ok: true,
                mensaje: "Marca creado/a con éxito.",
                alerta: 'success',
                registro
            });
        }
...
const actualizar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const registroActualizado = yield Marca_model_1.Marca.findByIdAndUpdate({ _id: id }, data, { new: true });
        ...
        return res.status(200).json({
            ok: true,
            mensaje: "Marca actualizado/a con éxito.",
            alerta: 'success',
            registroActualizado
        });
    }
...
const eliminar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const registro = yield Marca_model_1.Marca.findByIdAndDelete(id);
        if (!registro) {
            return res.json({
                ok: false,
                mensaje: "Marca no encontrado/a.",
            });
        }
        return res.status(200).json({
            ok: true,
            mensaje: "Marca eliminado/a con éxito.",
        });
    }
    catch (error) {
        console.log("======================================================");
        console.log("===================/Marca/eliminar===============");
        console.log(error);
        return res.send({
            ok: false,
            mensaje: "Algo salió mal.",
